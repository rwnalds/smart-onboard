...
import { AgencyConfig, Answer, SessionState, InputType } from '../types';
...
const OnboardingFlow: React.FC<Props> = ({ config, mode, onExit }) => {
  ...
  const loadNextQuestion = async (history: Answer[]) => {
    ...
    try {
      ...
      if (nextQ.isComplete && mode === 'client') {
        saveSubmission(history, nextQ.summary || "No summary generated.");
      }
      ...
      setSession(prev => ({ ...prev, isLoading: false, error: "Failed to load question." }));
    }
  };
      ...
      return (
        ...
        >
          <div className="max-w-xl w-full">
            ...
            >
              <CheckCircle2 className="h-12 w-12" style={{ color: primaryColor, opacity: 5 }} />
            </div>
            <h1 className="text-4xl md:text-5xl font-bold mb-6 tracking-tight">Thank you!</h1>
            <p className="text-xl mb-8 font-light opacity-70">
              Your information has been received. Our team at <span className="font-semibold">{config.name}</span> will review your needs and get back to you shortly.
            </p>
    ...
    return (
      ...
      >
        <div className="bg-white p-10 rounded-3xl shadow-2xl max-w-2xl w-full border border-gray-100">
          ...
          <p className="text-sm text-gray-400 mb-8 uppercase tracking-wide">Preview Mode</p>
          
          {session.currentQuestion.summary && (
            <div className="bg-gray-50 rounded-xl p-6 text-left mb-8 border border-gray-100">
              <h3 className="text-sm font-semibold text-gray-400 uppercase tracking-wider mb-2">AI Summary of Client</h3>
  ...
  if (session.isLoading && !session.currentQuestion) {
    return (
      ...
      >
        ...
        <p className="font-medium animate-pulse opacity-50" style={{ color: textColor }}>Thinking...</p>
      </div>
    );
  }
    ...
    return (
      ...
      >
         <div className="text-center">
             ...
             <button onClick={onExit} className="underline" style={{ color: textColor }}>Go Back</button>
         </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      {/* Progress Bar */}
      <div className="w-full h-1.5 fixed top-0 left-0 z-50 bg-opacity-10" style={{ backgroundColor: textColor, opacity: 0.1 }}>
        <div 
          ...
          >
            ...
            <div className="mt-8 mb-12">
              <FormInput 
                ...
                autoFocus={!isExiting && !session.isLoading}
            ...
            <div className="flex items-center gap-4">
              ...
              {!currentAnswer && session.currentQuestion.type !== InputType.BOOLEAN && (
